...
class PredictionRuntimeTests(TestCase):
    ...
    def test_invalid_market_json_is_controlled(self, mock_get):
        ...
        with self.assertRaises(ServiceUnavailableError):
            fetch_market_prices(
                '감자', '서울', '20260101', '20260102', codes, {'서울': ['1101', '108']}
            )
    ...
    def test_market_decodes_service_key_and_normalizes_json(self, mock_get):
                    ...
                    'body': {
                        ...
                        'items': {
                            'item': [
                                {
                                    'exmn_ymd': '20260101',
                                    'exmn_dd_cnvs_prc': '1,200',
                                    'item_nm': '감자',
                                },
                                {
                                    'exmn_ymd': '20260102',
                                    'exmn_dd_cnvs_prc': '1300',
                                    'item_nm': '감자',
                                },
                            ]
                        },
        ...
        frame = fetch_market_prices(
            '감자', '서울', '20260101', '20260102', self._market_codes(), {}
        )
        self.assertEqual(mock_get.call_args.kwargs['params']['serviceKey'], 'encoded+key')
                ...
                json=lambda: {
                    'response': {
                        ...
                        'body': {
                            'totalCount': 2,
                            'items': {
                                'item': [{'exmn_ymd': '20260101', 'exmn_dd_cnvs_prc': '100'}]
                            },
                        },
                    }
                },
                ...
                json=lambda: {
                    'response': {
                        ...
                        'body': {
                            'totalCount': 2,
                            'items': {
                                'item': [{'exmn_ymd': '20260102', 'exmn_dd_cnvs_prc': '200'}]
                            },
                        },
                    }
                },
    ...
    def test_market_api_error_timeout_and_invalid_price_are_controlled(self, mock_get):
        mock_get.return_value = Mock(
            status_code=200, json=lambda: {'response': {'header': {'resultCode': '30'}, 'body': {}}}
        )
        with self.assertRaises(ServiceUnavailableError):
            fetch_market_prices('감자', '서울', '20260101', '20260102', self._market_codes(), {})
    ...
    def test_price_features_use_only_prices_observed_by_feature_date(self):
        ...
        features, targets, prediction_input = _build_price_dataset(frame)

        feature_date_index = features.index[-1]
        self.assertEqual(
            features.loc[feature_date_index, 'price_lag_1'], frame.loc[feature_date_index, 'price']
        )
        self.assertEqual(
            targets.loc[feature_date_index], frame.loc[feature_date_index + 1, 'price']
        )
        self.assertEqual(prediction_input.iloc[0]['price_lag_1'], frame.iloc[-1]['price'])
    ...
    @patch(
        'prediction.views.fetch_market_prices', side_effect=ServiceUnavailableError('unavailable')
    )
    def test_session_details_survive_market_chart_outage(self, _mock_market):
        ...
        with self.assertLogs('prediction.views', level='WARNING'):
