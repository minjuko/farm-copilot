...
class SoilServiceTests(TestCase):
        ...
        mock_get.return_value = Mock(
            ...
            json=lambda: {
                'documents': [
                    {
                        'address': {
                            'address_name': '전북특별자치도 전주시 덕진구 중동 837',
                            'b_code': '5211313800',
                        },
                        'road_address': {
                            'address_name': '전북특별자치도 전주시 덕진구 농생명로 300'
                        },
                    }
                ]
            },
    ...
    def test_kakao_address_codes_include_pnu(self, mock_get):
        ...
        self.assertEqual(
            find_address_codes('sample'),
            {
                'stdg_code': '1234567890',
                'pnu_code': '1234567890100120003',
            },
        )
    ...
    def test_fertilizer_response_parsing(self, mock_get):
        ...
        self.assertEqual(
            ...
            'https://apis.data.go.kr/1390802/SoilEnviron_FrtlzrUse_V2/getSoilFrtlzrExamInfo',
        )
    ...
    def test_fertilizer_api_error_and_empty_are_controlled(self, mock_get):
        mock_get.return_value = Mock(
            status_code=200, content=b'<response><Result_Code>500</Result_Code></response>'
        )
        with self.assertRaises(ServiceUnavailableError):
            fetch_fertilizer(get_crop_names()[0], {}, '1234567890100100000')
        mock_get.return_value = Mock(
            status_code=200, content=b'<response><body><items /></body></response>'
        )
        with self.assertRaises(NotFoundError):
            fetch_fertilizer(get_crop_names()[0], {}, '1234567890100100000')
    ...
    def test_invalid_crop_mapping_is_controlled(self):
        with patch.dict(
            os.environ, {'DATA_GO_KR_FERTILIZER_V2_SERVICE_KEY': 'test-key'}, clear=True
        ):
            with self.assertRaises(ValidationError):
...
class SoilEndpointTests(TestCase):
    ...
    def test_address_search_endpoint_preserves_frontend_contract(self, mock_search):
        ...
        self.assertEqual(
            response.json()['normalized_query'], '전북특별자치도 전주시 덕진구 농생명로 300'
        )
    ...
    @patch(
        'soil.views.find_address_codes',
        return_value={'stdg_code': '1234567890', 'pnu_code': '1234567890100010000'},
    )
    @patch(
        'soil.views.fetch_fertilizer',
        return_value=([{'crop_Code': '01001', 'pre_Fert_N': '10'}], {'acid': '6'}),
    )
    def test_fertilizer_endpoint_preserves_frontend_contract(self, mock_fertilizer, mock_codes):
        ...
        saved = CropData.objects.get(user_id=self.user.id)
    ...
    def test_each_analysis_gets_an_independent_id(self, _mock_fertilizer, _mock_codes):
        ...
        self.assertEqual(CropData.objects.filter(user_id=self.user.id).count(), 2)

    def test_delete_removes_only_the_requested_analysis_owned_by_user(self):
        first = CropData.objects.create(user_id=self.user.id, session_id='analysis-1')
        second = CropData.objects.create(user_id=self.user.id, session_id='analysis-2')
        other_user = User.objects.create_user(
            email='soil-other@example.com', username='soil-other', password='test-password'
        )
        other = CropData.objects.create(user_id=other_user.id, session_id='analysis-1')
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(CropData.objects.filter(pk=first.pk).exists())
        self.assertTrue(CropData.objects.filter(pk=second.pk).exists())
        self.assertTrue(CropData.objects.filter(pk=other.pk).exists())
    ...
    def test_soil_detail_hides_another_users_analysis(self):
        ...
        CropData.objects.create(user_id=other_user.id, session_id='private-analysis')
